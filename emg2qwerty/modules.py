...
class TDSLSTMEncoderModified(nn.Module):
  def __init__(
    self,
    num_features: int,
    lstm_hidden_size: int = 128,
    ...
    self.layer_norm = nn.LayerNorm(lstm_hidden_size * 2)  # Layer normalization
...
class TDSGRUEncoder(nn.Module):
  ...
  def forward(self, inputs: torch.Tensor) -> torch.Tensor:
    x, _ = self.gru_layers(inputs) # (T, N, 2 * gru_hidden_size))
    x = self.fc_block(x)
    x = self.out_layer(x)
...
class CNNRNNEncoder(nn.Module):
  ...
  def forward(self, x):
      x = x.permute(0, 2, 1)  # Change to (batch_size, input_dim, seq_len) for CNN
      
      # Apply convolutions
      x = self.relu(self.batchnorm(self.conv1(x)))
      x = self.relu(self.batchnorm(self.conv2(x)))
      
      x = x.permute(0, 2, 1)  # Change back to (batch_size, seq_len, hidden_dim) for RNN
      
      output, _ = self.rnn(x)  # (T, N, hidden_dim * 2)
